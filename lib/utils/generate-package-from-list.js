...
module.exports = (filepaths, inputFolder, outputFile, options) => {
  ...
  Promise.all(filepaths
    ...
    .map(elem => { return typeFactory.getType(elem.split(path.sep)[0], elem, inputFolder, path.dirname(outputFile), options).package() })
  ).then(result => {
    let packageList = {};
    result.forEach(type => {
      for (let key in type) {
    ...
    saveFile(outputFile, xmlContent, options);
    return true;
  })
  .catch(err => {
    throw err;
