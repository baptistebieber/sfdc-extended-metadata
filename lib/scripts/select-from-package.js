...
module.exports = (inputPackage, inputFolder, outputFolder, options) => {
  return new Promise((resolve, reject) => {
    xml2jsAsync({content:fs.readFileSync(inputPackage)})
    .then(res => {
      ...
      Promise.all(
        ...
        var listFiles = [];
        res.map(elem => elem.map(filePath => listFiles.push(filePath)))
        console.log(listFiles.sort((a,b) => a.localeCompare(b)))
      })
    })
    .catch((err) => {
